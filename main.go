...
func main() {
	...
	flag.StringVar(&dockerBinary, "dockerBinary", "/usr/local/bin/docker", "docker binary")
	...
	if !asciiDisplay {
		...
		dockerRun := fmt.Sprintf("%v run --rm=true -p %v:%v -v /var/run/docker.sock:/var/run/docker.sock --name=%v %v x11vnc -geometry 640x480 -forever -usepw -create", dockerBinary, vncPort, vncPort, containerName, imageName)
		//dockerRun := fmt.Sprintf("%v run --rm=true -p %v:%v -v /var/run/docker.sock:/var/run/docker.sock -v %v:/dockerdoom.socket --name=%v %v x11vnc -geometry 640x480 -forever -usepw -create", dockerBinary, vncPort, vncPort, socketFile, containerName, imageName)
